  ...
  const indeedFunc = async () => {
    try {
      ...
      $(".jobsearch-SerpJobCard").each((i, el) => {
        ...
        const link = "https://www.indeed.co.uk" + header.attr("href")
...
// cvLibrary scraping
  const cvLibraryFunc = async () => {
    try {
      let url;
      if(sort === "relevance") url = `https://www.cv-library.co.uk/search-jobs?distance=15&geo=${city}&offset=0&order=sm&posted=&q=${job}&salarymax=&salarymin=&salarytype=annum&tempperm=Any`
      if(sort === "date") url = `https://www.cv-library.co.uk/search-jobs?distance=15&geo=${city}&offset=0&order=date&posted=&q=${job}&salarymax=&salarymin=&salarytype=annum&tempperm=Any`
      const response = await request(url)
      const jobsarray = []
      const $ = cheerio.load(response)
      $(".job.search-card").each((i, el) => {
        ...
        const company = $(el).find(".job__details-value.text--semibold").text().trim()
        const location = $(el).find(".job__details-location").text().trim()
        const salary = $(el).find(".job__details-value.salary").text().trim()
        const type = $(el).find(".job__details.l-flex-row dd[class='job__details-value']").text().trim()
